  ...
  return (
    ...
    >
      <div className="mb-16 text-center px-6">
        <p className="text-[#F56013] font-bold tracking-[0.2em] mb-4 uppercase">
      ...
      <div className="relative w-full">
        ...
        <div className="flex w-max gap-10 animate-marquee hover:[animation-play-state:paused] pl-8">
          {carouselItems.map((cat, i) => (
            <div
              key={i}
              className="group relative w-[280px] h-[360px] flex-shrink-0 cursor-pointer perspective-1000"
            >
              <div
                className={`absolute inset-0 rounded-[40px] bg-gradient-to-br ${cat.gradient}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 w-32 h-32 bg-black/10 rounded-full blur-2xl -translate-x-10 translate-y-10" />
              </div>

              <div className="relative h-full flex flex-col items-center justify-end p-6 pb-8 z-10">
                <div className="absolute top-8 left-1/2 -translate-x-1/2 w-48 h-48 transition-all duration-500 group-hover:scale-125 group-hover:-translate-y-12 drop-shadow-2xl">
                  <div className="w-full h-full rounded-3xl overflow-hidden border-4 border-white/20 shadow-inner rotate-3 group-hover:rotate-0 transition-all duration-500 relative">
                    <img
                      src={cat.image}
                      alt={cat.title}
                      className="w-full h-full object-cover"
                      onError={() => handleImageError(i, cat.image)}
                    ...
                    {imageErrors[i] && (
                      <div 
                        className={`absolute inset-0 w-full h-full bg-gradient-to-br ${cat.gradient} flex items-center justify-center`}
                      >
                ...
                <div className="text-center transition-all duration-500 group-hover:translate-y-2">
                  <h3 className="text-2xl font-heading uppercase text-white mb-2 drop-shadow-md">
                    {cat.title}
                  </h3>
                  <p className="text-white/90 font-bold text-sm uppercase tracking-wider bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm">
                    {cat.hindi}
                  </p>
                </div>
              </div>
